...
var JADependForm = function () {
	...
	self.hideOptionJ4 = function () {
		jQuery(document).ready(function () {
			if (jVer >= 4) {
				var comK2Label = jQuery('div.field-spacer');
				var k2CloneImgs = jQuery('div.control-label');
				var comOptions = jQuery('select#jform_re_translate');
				if (comK2Label.length) {
					comK2Label.remove();
				}

				if (k2CloneImgs.find('label').length) {
					jQuery.each(k2CloneImgs.find('label'), function (idx, el) {
						var el = jQuery(el);
						if (el.text().trim().toLowerCase().includes('clone image')) {
							el.parent().parent().remove();
						}
					})
				}

				if (comOptions.length) {
					jQuery.each(comOptions.find('option'), function (idx, el) {
						if (jQuery(el).attr('value') === 'k2_items'
							|| jQuery(el).attr('value') === 'mijoshop_product_description') {
							jQuery(el).remove();
						}
					})
				}
